...
#define left_pwm 9  // PWM
#define right_pwm 11  // PWM
#define left_dir_relay 6
#define right_dir_relay 7
...
void setup() {
  // Init Connection PINs
  pinMode(right_pwm, OUTPUT);
  pinMode(left_pwm, OUTPUT);
  pinMode(right_dir_relay, OUTPUT);
  pinMode(left_dir_relay, OUTPUT);
...
void loop() {
  ...
  if (Serial.available())
  {
    ...
    else if(chr == 'p')
    {
      if(is_right_wheel_cmd && !is_right_wheel_forward)
      {
        ...
        digitalWrite(right_dir_relay, LOW);
      ...
      else if(is_left_wheel_cmd && !is_left_wheel_forward)
      {
        ...
        digitalWrite(left_dir_relay, HIGH);
    ...
    else if(chr == 'n')
    {
      ...
      {
        // change the direction of the rotation
        digitalWrite(right_dir_relay, HIGH);
        is_right_wheel_forward = false;
      }
      else if(is_left_wheel_cmd && is_left_wheel_forward)
      {
        // change the direction of the rotation
        digitalWrite(left_dir_relay, LOW);
  ...
  {
    right_wheel_meas_vel = (10 * right_encoder_counter * (60.0/514.8)) * 0.10472;
    left_wheel_meas_vel = (10 * left_encoder_counter * (60.0/514.8)) * 0.10472;
